...
form.addEventListener('click', (e) => {
  ...
  const surname = document.getElementById('surname').value;
  const day = parseInt(document.getElementById('dateofbirth').value);
  const month = parseInt(document.getElementById('month').value);
  const year = parseInt(document.getElementById('year').value);

  const text = `hi ${name} ${surname} , Your zodiac sign is ${zodiacSigns[month - 1]}. ${compliments[day - 1]}. ${victimCardCompliments[year % 20]
    }. ${recommendations[(day * month) % 30]}. ${predictions[name.length * surname.length]} `;
